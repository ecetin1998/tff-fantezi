...
function applyRelativeFixtureLevels(matches=[]){
  const sides=[]
  for(const m of matches){
    sides.push({matchId:m.match_id,side:'home',value:Number(m.home_elo_expectancy||0)})
    sides.push({matchId:m.match_id,side:'away',value:Number(m.away_elo_expectancy||0)})
  }
  ...
  sides.forEach((s,i)=>{
    const level=i<easyCount?'easy':i<mediumEnd?'medium':'hard'
    levelMap.set(`${s.matchId}:${s.side}`,level)
  })
  return matches.map(m=>({
    ...m,
    home_fixture_level:levelMap.get(`${m.match_id}:home`)||'medium',
    away_fixture_level:levelMap.get(`${m.match_id}:away`)||'medium',
  }))
}
...
export async function getMatches() {
  ...
  return {
    run,
    eloThroughGameweek,
    matches:applyRelativeFixtureLevels(enriched)
  }
}
