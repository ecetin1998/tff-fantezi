  ...
  return <>
    <div className="filters player-filters">
      ...
      <select className="mobile-sort-select" value={sort} onChange={e=>{setSort(e.target.value);setDir(-1)}}>
        <option value="xfp">xFP'ye göre</option><option value="xi">İlk 11 ihtimaline göre</option><option value="minutes">Dakikaya göre</option><option value="p90">Tavana göre</option><option value="six">6+ ihtimaline göre</option><option value="points">Toplam puana göre</option><option value="value">F/P'ye göre</option><option value="price">Fiyata göre</option>
      </select>
    </div>

    <div className="table-summary"><b>{rows.length}</b> oyuncu • detay için oyuncuya dokun</div>
    <div className="projection-legend">Karar metrikleri: <b>İlk 11</b> + <b>xDakika</b> oynama ihtimalini, <b>xFP</b> beklenen puanı, <b>Temkinli/Tavan</b> olası puan aralığını gösterir.</div>
    ...
    <div className="card table-wrap desktop-player-table"><table><thead><tr>
      ...
      {head('p25','Temkinli')}{head('p90','Tavan')}{head('six','6+ %')}{head('xg','xG')}{head('xa','xA')}{head('value','F/P')}
