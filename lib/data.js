...
async function _getCurrentRun() {
  ...
  const { data } = await supabase.from('scout_model_runs').select('id,gameweek,model_version,generated_at,source_updated_at,simulation_count,status,is_current').eq('is_current', true).order('gameweek', { ascending: false }).limit(1).maybeSingle()
  return data
}

async function _getPlayersWithProjection(limit = 700) {
  const supabase = createPublicClient()
  ...
  const [{ data: players }, { data: projections }, { data: availability }, { data: seasonStats }] = await Promise.all([
    ...
    supabase.from('scout_availability').select('player_id,availability_type,reason,checked_at,availability_probability,source_reason,injury_date,expected_return,suspension_fixture').eq('run_id', run.id),
...
async function _getAvailability() {
  const supabase=createPublicClient(), run=await getCurrentRun()
  if(!run) return {run:null,rows:[]}
  const {data:rows}=await supabase.from('scout_availability').select('run_id,player_id,availability_type,availability_probability,reason,checked_at,source_url,suspension_end,source_reason,injury_date,expected_return,suspension_fixture').eq('run_id',run.id).order('availability_probability',{ascending:true})
...
async function _getPlayerDetail(playerId) {
  ...
  const [
    ...
    run ? supabase.from('scout_availability').select('run_id,player_id,availability_type,availability_probability,reason,checked_at,source_url,suspension_end,source_reason,injury_date,expected_return,suspension_fixture').eq('run_id', run.id).eq('player_id', id).maybeSingle() : Promise.resolve({ data:null }),
...
async function _getWeeklyPoints() {
  ...
  const [{ data: players }, { data: teams }, { data: stats }] = await Promise.all([
    ...
    supabase.from('scout_player_season_stats').select('player_id,actual_points,matches_played,minutes,six_plus_count').limit(700),
  ])
...
async function _getTeamDetail(teamId) {
  ...
  const [{ data: projections }, { data: availability }] = await Promise.all([
    ...
    run && playerIds.length
      ? supabase.from('scout_availability').select('run_id,player_id,availability_type,availability_probability,reason,checked_at,source_url,suspension_end,source_reason,injury_date,expected_return,suspension_fixture').eq('run_id',run.id).in('player_id',playerIds)
      : Promise.resolve({data:[]}),
  ])
