...
export async function getBacktestOverview() {
  const supabase = await createClient()
  const [{ data: weeks }, { data: learning }] = await Promise.all([
  ...
  const latestClosed=[...(weeks||[])].reverse().find(w=>w.status==='closed')
  let latestPlayers=[]
  if(latestClosed){
    const {data}=await supabase.from('scout_backtest_players')
      ...
      .eq('gameweek',latestClosed.gameweek)
  ...
  return {weeks:weeks||[],learning:learning||[],latestPlayers}
}
