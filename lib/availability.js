...
export const availabilityExpectedReturn=value=>{
  ...
  if(!text)return ''
  return text.replace(/^(.+?) ayı (başında|ortasında|sonunda) (\d{4})$/i,(_m,month,when,year)=>`${month} ${year} ${{başında:'başı',ortasında:'ortası',sonunda:'sonu'}[when.toLocaleLowerCase('tr')]||when}`)
}
